...
function __hhs_classic_minput_curpos() {
...
function __hhs_classic_minput() {

  local outfile title ret_val=1 label_size=10 value_size=30 all_fields=() cur_field=() field_parts=() all_pos=()
  local f_label f_mode f_type f_max_min_len f_perm f_value f_row f_col f_pos err_msg dismiss_timeout re_render=1
  local len min_len offset margin max_len idx tab_index cur_row cur_col val_regex mi_modes=() mi_types=() file_contents
  ...
  if [[ $# -lt 2 || "$1" == "-h" || "$1" == "--help" ]]; then
    ...
    echo ''
    echo '    Fields: '
    echo "            <Label> : The field label. Consisting only of alphanumeric characters and under‐scores."
  ...
  title="${title:-Please fill all fields of the form below}"
  ...
  while :; do
    ...
    if [[ -n "$re_render" ]]; then
      ...
      for idx in "${!all_fields[@]}"; do
        ...
        if [[ ${tab_index} -ne ${idx} ]]; then
          ...
          f_pos="${all_pos[idx]:-$(__hhs_classic_minput_curpos)}" # Buffering the all positions to avoid calling __hhs_classic_minput_curpos
          f_row="${f_pos%,*}" && f_col="${f_pos#*,}"
          f_col="$((f_col + ${#f_value}))"
          all_pos[idx]="${f_pos}"
        fi
        offset=${#f_value}
        margin=$((10 - (${#max_len} + ${#offset})))
        ...
        printf " ${icon}  %d/%d" "${#f_value}" "${max_len}"
      ...
      echo -en "${YELLOW}[Enter] Submit  [↑↓] Navigate  [Tab] Next  [Esc] Quit\033[0K"
      echo -en "${NC}"
      unset re_render
    fi
    # } Menu Renderization
    ...
    case "${keypress}" in
      $'\011') # Handle TAB => Validate and move next. First case statement because next one also captures it
        min_len=${cur_field[3]%/*}
        if [[ ${min_len} -le ${#cur_field[5]} ]]; then
          ...
          err_msg="This field does not match the minimum length of ${min_len}"
        fi
        ;;
      ...
      [[:alpha:]] | [[:digit:]] | [[:space:]] | [[:punct:]]) # Handle an input
        ...
        if [[ "rw" == "${cur_field[4]}" && "${f_mode}" == "checkbox" ]]; then
          ...
          all_fields[${tab_index}]="${cur_field[0]}|${cur_field[1]}|${cur_field[2]}|${cur_field[3]}|${cur_field[4]}|${cur_field[5]}"
        elif [[ "rw" == "${cur_field[4]}" && ${#cur_field[5]} -lt max_len ]]; then
          case "${f_type}" in
            'letter') val_regex='^[a-zA-Z ]*$' ;;
            'number') val_regex='^[0-9]*$' ;;
            'alphanumeric') val_regex='^[a-zA-Z0-9 ]*$' ;;
            *) val_regex='.*' ;; # FIXME This expression is rejecting á, ç and similar.
          esac
      ...
      $'') # Validate & Save the form and exit
        ...
        for idx in ${!all_fields[*]}; do
          ...
          if [[ ${#f_value} -lt ${min_len} ]]; then
            err_msg="Field \"${f_label}\" doesn't meet the required length of ${min_len} !"
            tab_index=${idx}
            unset file_contents
            break
          else
