...
for file in $(find "${HHS_HOME}/bin/hhs-functions/bash" -maxdepth "${HHS_MAXDEPTH}" -type f -name "hhs-*.bash" | sort); do
  source "$file"
done

# Load all functions that were previously aliased
for file in $(find "$HHS_HOME/bin/ext-tools/bash" -maxdepth "${HHS_MAXDEPTH}" -type f -name "*.bash" | sort); do
  source "$file"
done
