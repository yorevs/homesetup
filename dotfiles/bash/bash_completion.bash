...
case "${HHS_MY_SHELL}" in
  ...
  bash)
    ...
    if [[ -f "$AUTO_CPL_D/ssh-completion.bash" ]]; then
      \. "$AUTO_CPL_D/ssh-completion.bash"
      AUTO_CPL_TYPES+=('ssh')
    fi
  
    ;; # End of bash completions
  
  *)
    echo "${HHS_MY_SHELL} shell does not include any cmpletions" >>"${HHS_WARNINGS_FILE}"
    ;;
